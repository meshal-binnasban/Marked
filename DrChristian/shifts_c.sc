...
// Mark(n, m, from, to):
//   n,m     = total matched slice of the input: s[n..m)
//   from,to = slice of the input advanced by the current shifts call: s[from..to)
case class Mark(n: Int, m: Int, from: Int = 0, to: Int = 0)


type Marks = Set[Mark]

def shifts(ms: Marks, s: String, r: Rexp): Marks =
  ms.flatMap(p => shifts1(p, s, r))

def shifts1(p: Mark, s: String, r: Rexp): Marks = {
  val Mark(n0, m0, _, _) = p
  r match {
    case ZERO =>
      Set.empty
    ...
    case CHARS(c, id) =>
      if (m0 < s.length && s(m0) == c)
        Set(Mark(n0, m0 + 1, m0, m0 + 1))
      else
        Set.empty

    case ALTS(r1, r2, id) =>
      shifts1(p, s, r1) ++ shifts1(p, s, r2)

    case SEQS(r1, r2, id) =>
      val out1 = shifts1(p, s, r1)

      val inR2 =
        if (nullable(r1)) out1 + p
        else out1

      inR2.foreach(q => recordSplit(id, q.m))

      val out2 =
        if (nullable(r2)) shifts(inR2, s, r2) ++ out1
        else               shifts(inR2, s, r2)

      out2.map { q =>
        val from = math.min(p.from, q.from)
        val to   = math.max(p.to,   q.to)
        Mark(q.n, q.m, from, to)
      }

    case STARSS(rBody, id) =>
      val out1 = shifts1(p, s, rBody)

      out1.foreach(q => recordSplit(id, q.m))

      val out =
        if (out1.isEmpty) Set.empty
        else out1 ++ shifts(out1, s, STARSS(rBody, id))
    ...
    case NTIMES(rBody, k) =>
      val out =
        if (k == 0) Set.empty
        else if (k == 1) shifts1(p, s, rBody)
        else {
          val out1 = shifts1(p, s, rBody)
          if (out1.isEmpty) Set.empty
          else if (nullable(rBody))
            out1 ++ shifts(out1, s, NTIMES(rBody, k - 1))
          else
            shifts(out1, s, NTIMES(rBody, k - 1))
        }
...
val invalid = (Invalid, Mark(-1, -1, -1, -1))
...
def back(r: Rexp, s: String, p: Mark, splits: Map[Int, Set[Int]]): (Val, Mark) =
  r match {
    ...
    case SEQS(r1, r2, id) =>
      ...
      val r1All = if (!nullable(r2)) invalid
      ...
      val r1r2: (Val, Mark) = kSplits.map { k =>
      ...
      (nullable(r1), nullable(r2)) match {
        case (true,  true)  => if (!isInvalid(r1All._2)) r1All
                                else if (!isInvalid(r1r2._2)) r1r2
                                else r2All
    ...
    case STARSS(rBody, id) =>
      ...
      def decodeStar(a: Int, b: Int): (Val, Mark) =
        if (a == b) (Stars(Nil), Mark(a, a))
          ...
          kSplits.map { k =>
              back(rBody, s, Mark(a, k), splits) match {
                case (v1, p1) if !isInvalid(p1) =>
...
def testExamples(): Unit = {
  ...
  tests.zipWithIndex.foreach { case ((label, r, s), i) =>
    ...
    if (s == "") {
        val vm: Val = if (nullable(r)) Empty else Invalid
        (vm, Set.empty[Mark])
    } else {
