...
import ARexp._, Bit._
import Rexp._
import Val._

// standard regular expressions including n-times
/* enum Rexp {
  case ZERO extends Rexp
  case ONE extends Rexp
  case CHAR(c: Char) extends Rexp
  case ALT(r1: Rexp, r2: Rexp) extends Rexp
  case SEQ(r1: Rexp, r2: Rexp) extends Rexp
  case STAR(r: Rexp) extends Rexp
  case OPTIONAL(r: Rexp) extends Rexp
  case NOT(r: Rexp) extends Rexp
  case NTIMES(r: Rexp, n: Int) extends Rexp
} */


// annotated regular expressions
enum ARexp {
  case AZERO extends ARexp
  case AONE(bs: Bits) extends ARexp
  case ACHAR(bs: Bits, c: Char) extends ARexp
  case AALTS(bs: Bits, rs: List[ARexp]) extends ARexp
  case ASEQ(bs: Bits, r1: ARexp, r2: ARexp) extends ARexp
  case ASTAR(bs: Bits, r: ARexp) extends ARexp
  case AOPTIONAL(bs: Bits, r: ARexp) extends ARexp
  case ANOT(bs: Bits, r: ARexp) extends ARexp
  case ANTIMES(bs: Bits, r: ARexp, n: Int) extends ARexp
}
...
// values
/* enum Val {
    case Empty extends Val
    case Chr(c: Char) extends Val
    case Sequ(v1: Val, v2: Val) extends Val
    case Left(v: Val) extends Val
    case Right(v: Val) extends Val
    case Stars(vs: List[Val]) extends Val
    case Not(r: Rexp) extends Val
    case Nt(vs: List[Val], n: Int) extends Val
} */
...
given string2rexp: Conversion[String, Rexp] with {
    def apply(s: String): Rexp = charlist2rexp(s.toList)
}
...
def erase(r: ARexp): Rexp = r match {
  ...
  case AOPTIONAL(cs, r) => OPTIONAL(erase(r))
  case ANOT(cs, r) => NOT(erase(r))
...
def decode_aux(r: Rexp, bs: Bits): (Val, Bits) = {
  (r, bs) match {
    ...
    case (STAR(_), S :: bs) =>
      (Stars(Nil), bs)
    case (OPTIONAL(_), Z :: bs) =>
      (Left(Empty), bs)
    case (OPTIONAL(r1), S :: bs) =>
      val (v, bs1) = decode_aux(r1, bs)
      (Right(v), bs1)
    case (NOT(r1), bs) =>
      (NotV(r1), bs)
    case (NTIMES(r1, n), bs) =>
      decode_aux(STAR(r1), bs) match {
        case (Stars(vs), bs1) => (Nt(vs, n), bs1)
      }
  }
}
...
def decode2(r: Rexp, bs: Bits, k: (Val, Bits) => (Val, Bits)): (Val, Bits) =
  (r, bs) match {
    ...
    case (ALT(r1, r2), S :: bs) =>
      decode2(r2, bs, (v, bs) => k(Right(v), bs))
    case (SEQ(r1, r2), bs) =>
    ...
    case (OPTIONAL(_), Z :: bs) =>
      k(Left(Empty), bs)
    case (OPTIONAL(r1), S :: bs) =>
      decode2(r1, bs, (v, bs1) => k(Right(v), bs1))
    case (NOT(r1), bs) =>
      k(NotV(r1), bs)
    case (NTIMES(r1, n), bs) =>
      decode2(STAR(r1), bs, { case (Stars(vs), bs1) => k(Nt(vs, n), bs1) })
  }
...
enum RegStack {
  case CEMPTY extends RegStack
  case CLEFT(k: RegStack) extends RegStack
  case CRIGHT(k: RegStack) extends RegStack
  case CSEQ1(r: Rexp, k: RegStack) extends RegStack
  case CSEQ2(v: Val, k: RegStack) extends RegStack
  case CSTARS(r: Rexp, vs: List[Val], k: RegStack) extends RegStack
  case CNTIMES(n: Int, k: RegStack) extends RegStack
}
...
def decode3(r: Rexp, bs: Bits, k: RegStack): (Val, Bits) = {
  (r, bs) match {
    ...
    case (OPTIONAL(_), Z :: bs) => pop_and_decode(k, Left(Empty), bs)
    case (OPTIONAL(r1), S :: bs) => decode3(r1, bs, CRIGHT(k))
    case (NOT(r1), bs) => pop_and_decode(k, NotV(r1), bs)
    case (NTIMES(r, n), bs) => decode3(STAR(r), bs, CNTIMES(n, k))
  }
}
...
def pop_and_decode(k: RegStack, v: Val, bs: Bits): (Val, Bits) = {
  k match {
    ...
    case CNTIMES(n, rest) => v match {
      case Stars(vs) => pop_and_decode(rest, Nt(vs, n), bs)
    }
  }
}
...
enum Call {
  case Dec(r: Rexp, bs: Bits, k: RegStack) extends Call
  case Pop(k: RegStack, v: Val, bs: Bits) extends Call
}
...
def de3(c: Call): (Val, Bits) = c match {
  case Dec(r: Rexp, bs: Bits, k: RegStack) =>
    (r, bs) match {
      ...
      case (OPTIONAL(_), Z :: bs) => de3(Pop(k, Left(Empty), bs))
      case (OPTIONAL(r1), S :: bs) => de3(Dec(r1, bs, CRIGHT(k)))
      case (NOT(r1), bs) => de3(Pop(k, NotV(r1), bs))
      case (NTIMES(r, n), bs) => de3(Dec(STAR(r), bs, CNTIMES(n, k)))
    }
  case Pop(k: RegStack, v: Val, bs: Bits) =>
    k match {
      ...
      case CNTIMES(n, rest) => v match {
        case Stars(vs) => de3(Pop(rest, Nt(vs, n), bs))
      }
    }
}
...
def bsimp(r: ARexp): ARexp = r match {
  ...
  case AOPTIONAL(bs1, r1) =>
    bsimp(AALT(bs1, fuse(List(Z), AONE(Nil)), fuse(List(S), bsimp(r1))))
  case ANOT(bs1, r1) =>
    ANOT(bs1, bsimp(r1))
  case r => r
}
...
def size(r: Rexp): Int = r match {
  ...
  case OPTIONAL(r1) => 1 + size(r1)
  case NOT(r1) => 1 + size(r1)
...
// and BitCode annotations by Sulzmann and Lu
//
//   tested with 
//
//     Ammonite Repl 2.5.8 (Scala 3.2.2 Java 17.0.1)
//
//   call with
//
//   amm re-bitrev.sc     
...
//import rexp._, ARexp._, Val._, Bit._    
import ARexp._, Bit._ 
