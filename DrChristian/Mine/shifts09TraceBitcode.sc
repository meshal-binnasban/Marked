...
def shifts(ms: Marks, trace: Array[Trace], s: String, r: Rexp): (Marks, Array[Trace]) = {
  ...
  r match {
    ...
    case SEQ(r1, r2) =>
      ...
      for (m <- ms1.sorted.reverse) { // try furthest splits first
        traceR2(m) = Nil
    ...
    case STAR(r) =>
      val starOutTrace = new Array[Trace](s.length + 1)
      
      //zero repitions
      for (m <- ms) starOutTrace(m) = trace(m) ++ List(1)

      val nextStarTrace = new Array[Trace](s.length + 1)
      var mss: List[Int] = Nil

      val rFreshTrace = new Array[Trace](s.length + 1)

      for (start <- ms) {
        rFreshTrace(start) = Nil
        val (ms1, tr1) = shifts(List(start), rFreshTrace, s, r)

        
        for (b <- ms1.sorted.reverse) {
          if (nextStarTrace(b) == null && starOutTrace(b) == null ) {
            nextStarTrace(b) = trace(start) ++ (0 :: tr1(b))
            mss = b :: mss
          }
        }

        rFreshTrace(start) = null
      }

      if (mss.isEmpty) {
        (ms, starOutTrace)
      } else {
        val (msStar, trStar) = shifts(mss.sorted.reverse, nextStarTrace, s, STAR(r))

        for (b <- msStar) {
          if (starOutTrace(b) == null) starOutTrace(b) = trStar(b)
        }

        ((ms ::: msStar).distinct, starOutTrace)
      }

    case NTIMES(r, 0) =>
      val outTrace = new Array[Trace](s.length + 1)
      for (m <- ms) outTrace(m) = trace(m) ++ List(1)
      (ms, outTrace)

    case NTIMES(r, n) =>
      val nextTrace = new Array[Trace](s.length + 1)
      var mss: List[Int] = Nil
      val traceR = new Array[Trace](s.length + 1)
      ...
      for (start <- ms) {
        ...
        for (b <- ms1.sorted.reverse) {
          if (nextTrace(b) == null) {
            nextTrace(b) = trace(start) ++ (0 :: tr1(b))
            mss = b :: mss
          }
        }
      ...
      if (mss.isEmpty) (Nil, new Array[Trace](s.length + 1))
      else shifts(mss.sorted.reverse, nextTrace, s, NTIMES(r, n - 1))

    case OPTIONAL(r) => // treat it as ALT(r,ONE) for trace, so left 0 for skipping and 1 for matching r.
      val outTrace = new Array[Trace](n + 1)
      var mss: List[Int] = Nil

      for (m <- ms) {
        outTrace(m) = trace(m) ++ List(0)
        mss = m :: mss
      }

      val (ms1, tr1) = shifts(ms, trace, s, r)
      for (b <- ms1) {
        if (outTrace(b) == null) {
          outTrace(b) = 1 :: tr1(b)
          mss = b :: mss
  ...
  (r: @unchecked) match {
    ...
    case STAR(r) =>
      (tr: @unchecked) match {
        ...
        case 0 :: rest =>
          val (v, s1, tr1) = back_aux(r, s, rest)
          val (vs, s2, tr2) = back_aux(STAR(r), s1, tr1)
          vs match {
    ...
    case NTIMES(r, n) =>
      (tr: @unchecked) match {
        case 0 :: rest =>
          val (v, s1, tr1) = back_aux(r, s, rest)
          val (vs, s2, tr2) = back_aux(NTIMES(r, n - 1), s1, tr1)
          vs match {
            case Nt(vs1, _) => (Nt(v :: vs1, n), s2, tr2)
          }
      }
    case OPTIONAL(r) =>
       (tr: @unchecked) match {
        case 0 :: rest => (Left(Empty), s, rest)
        case 1 :: rest =>
          val (v, s1, tr1) = back_aux(r, s, rest)
          (Right(v), s1, tr1)
      }
