//SOURCECODE_ORIGINAL_FILE_PATH=/Users/meshalbinnasban/Google Drive/KCL/Code Playground/Marked/DrChristian/shifts10Trace.scala
...
def matG(r: Rexp, s: String): GMarks = {
  ...
  def shifts2G(ms: GMarks, r: Rexp): GMarks = {
    calls += 1
    totalGroups += ms.size
    totalMarks += ms.toList.map(_.size).sum
    ...
    r match {
      ...
      case CHAR(c) =>
        ms.map { group =>
          for (m <- group if m < s.length && s(m) == c) yield m + 1
        }.filter(_.nonEmpty)

      case ALT(r1, r2) => shifts2G(ms, r1) ++ shifts2G(ms, r2)
      case SEQ(r1, r2) => shifts2G(shifts2G(ms, r1), r2)
      
      case STAR(r) =>
        //basically group marks from same iteration into a set and shift it into Star together 
        val ms1 = Set(shifts2G(ms, r).flatten.diff(ms.flatten)).filter(_.nonEmpty)
        println(s"ms1=$ms1 <- , ms=$ms in STAR")
        if (ms1.isEmpty) ms
        else {
          val out = ms ++ shifts2G(ms1, STAR(r))
          println(s"returned =$out in STAR")
          out
        }
      case STAR(r) =>
        val ms1 = shifts2G(ms, r).diff(ms)

        println(s"ms1=$ms1, , , , ms=$ms in STAR")
        if (ms1.isEmpty) ms 
        else
          val x=ms ++ shifts2G(ms1, STAR(r))
          println(s"returned =$x in STAR") 
          ///ms ++ shifts2G(ms1, STAR(r))
          x

      case NTIMES(r, n) =>
        if (n == 0) ms else shifts2G(shifts2G(ms, r), NTIMES(r, n - 1))

    
      case NOT(r) =>
        ms.flatMap { group =>
          val allFromGroup = Range(group.min, s.length + 1).toSet
          val rFromGroup = shifts2G(Set(group), r).flatten
          val out = allFromGroup.diff(rFromGroup)
          out.map(m => Set(m))
        }
...
def matcherG(r: Rexp, s: String): Boolean =
  if (s == "") nullable(r)
  else matG(r, s).flatten.contains(s.length)
...
type main = mainargs.main; private def shifts10Trace_scala_millScriptMainSelf = this; object MillScriptMain_shifts10Trace_scala { def main(args: Array[String]): Unit = this.getClass.getMethods.find(m => m.getName == "main" && m.getParameters.map(_.getType) == Seq(classOf[Array[String]]) && m.getReturnType == classOf[Unit]) match{ case Some(m) => m.invoke(shifts10Trace_scala_millScriptMainSelf, args); case None => mainargs.Parser(shifts10Trace_scala_millScriptMainSelf).runOrExit(args) }}