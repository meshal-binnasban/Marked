//SOURCECODE_ORIGINAL_FILE_PATH=/Users/meshalbinnasban/Google Drive/KCL/Code Playground/Marked/DrChristian/enumerate.scala
...
def decodeRegex[T](n: BigInt)(using cdata: CDATA[T]) : T ={
  val h = get_height(n)
  dec(h, n)
}
 ...
 */
type main = mainargs.main; private def enumerate_scala_millScriptMainSelf = this; object MillScriptMain_enumerate_scala { def main(args: Array[String]): Unit = this.getClass.getMethods.find(m => m.getName == "main" && m.getParameters.map(_.getType) == Seq(classOf[Array[String]]) && m.getReturnType == classOf[Unit]) match{ case Some(m) => m.invoke(enumerate_scala_millScriptMainSelf, args); case None => mainargs.Parser(enumerate_scala_millScriptMainSelf).runOrExit(args) }}