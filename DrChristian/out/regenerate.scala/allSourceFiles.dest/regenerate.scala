//SOURCECODE_ORIGINAL_FILE_PATH=/Users/meshalbinnasban/Google Drive/KCL/Code Playground/Marked/DrChristian/regenerate.scala
...
type main = mainargs.main; private def regenerate_scala_millScriptMainSelf = this; object MillScriptMain_regenerate_scala { def main(args: Array[String]): Unit = this.getClass.getMethods.find(m => m.getName == "main" && m.getParameters.map(_.getType) == Seq(classOf[Array[String]]) && m.getReturnType == classOf[Unit]) match{ case Some(m) => m.invoke(regenerate_scala_millScriptMainSelf, args); case None => mainargs.Parser(regenerate_scala_millScriptMainSelf).runOrExit(args) }}