...
branch assumes it is skipped and instead takes the derivative of $r_1$ directly. 
...
\begin{figure}[ht]
  \begin{center}
    ...
    \mbox{}
    \rule{\linewidth}{0.4pt}
   \caption{$\shift$ function in the Scala implementation of 
...
Suzuki~\cite{OkuiSuzuki2013}, although this ordering is not embedded within 
the shifting process itself. Instead, all possible values are generated first,
and then the ordering function is applied to select the POSIX value.
This line of thought eventually led us to the version of the 
...
passed to the left subexpression and $1$ to the right subexpression. If the input character 
matches a leaf character node, it is wrapped by the newly defined \POINT\ 
constructor, which represents the mark. The associated bitsequence is stored
inside this constructor together with the character.  
...
\renewcommand{\arraystretch}{1.0}
\]
where $bs'$ is the updated bitsequence. The complete definition appears in Figure~\ref{bitAnnotatedShiftfunction}.
...
\begin{itemize}
  \item If the input character $c$ matches $d$ and the flag $m$ is true, then a \POINT\ wraps the constructor $c$, 
        with the updated bitsequence $bs$ stored inside the \POINT. Otherwise, the character remains unmarked.
\end{itemize}
...
bitsequence is overwritten which can lead to value erasure and, in particular, to the loss of the POSIX value,  
...
\begin{itemize}
  \item The list of marks $ms$ is first shifted into $r$, producing a new list $ms'$.  
        ...
        Otherwise, $ms'$ records one completed iteration and is appended to the result of shifting $ms'$ into the \Star, 
...
example considers the \Star\ case, highlighting how the marks are unfolded. A third
...
when the \Star\ expression is part of a larger expression.
...
string \texttt{aaa}. For the \Star\ case, the process starts by shifting the received marks into
...
Second, it enumerates all the ways the \Star\ expression can match the input, from no consumption 
...
alternatives, and, as Example~3 will show, one of these marks may be the only one required to complete
 the match when the \Star\ expression is part of a larger expression. This observation also 
