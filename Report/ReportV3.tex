...
The marked approach offers an alternative to derivatives for regular expression 
matching. It relies on propagating marks within the regular expression rather than 
constructing new subexpressions. Our main motivation is that this method could support 
fast and high-performance matching with POSIX value extraction, since it handles 
matching in a way that avoids some of the limitations of the derivative-based approach.  

In the derivative method—for example, in the sequence case—the size of the expression 
typically increases due to the creation of new subexpressions, which contributes to 
the size explosion problem. By contrast, in the marked approach, matching achieves 
a similar result by propagating marks through the regular expression without 
generating larger expressions. We also hope that these marks can be used to extract 
POSIX values in an efficient manner.  
...
As noted earlier, the empty string $\ONE$ is not marked. This choice ensures 
...
the first three unfoldings are as follows: 
\[
\begin{aligned}
\text{step 1}:&\quad \Marked{aa}\; \At\; \Marked{aa} \quad (2) \\[4pt]
\text{step 2}:&\quad
  \Marked{a}\; \At\; \Marked{a}\; \At\; \Marked{a}\; \At\; \Marked{a} \quad (4) \\[4pt]
\text{step 3}:&\quad
  \underbrace{\Marked{\emptylist}\; \At\; \cdots\; \At\; \Marked{\emptylist}}_{8\ \text{marks}}
\end{aligned}
\]
...
their remaining strings. At present, the reordering is carried out in the
 \SEQText\ case after shifting through the first part.
...
\begin{figure}[ht]
  ...
  \mbox{}
  \rule{\linewidth}{0.4pt}
 \caption{$\shifts$ function. A mark is represented 
...
\begin{itemize}
  ...
  \item \textbf{Extending support for additional operators.}  
    Future work will focus on extending the matcher to handle intersection, 
    negation, and lookahead operators. This will require extending both the 
...
\begin{figure}[ht]
  \begin{center}
    ...
    \begin{tabular}{lcl}
      ...
      \multicolumn{3}{@{\hspace{15mm}}l}{
      $\begin{cases}
      \shift(m,\, (bs \lessdot \mkeps(r_1)\, \listconcat\ \mkfin(r_1)),\, c,\, r_2)
        & \text{if } m \land \nullable(r_1) \land \fin(r_1) \\
      \shift(m,\, bs \lessdot \mkeps(r_1),\, c,\, r_2)
        & \text{if } m \land \nullable(r_1) \\
      ...
      \multicolumn{3}{@{\hspace{15mm}}l}{
      $\begin{cases}
      \big(\shift(m,\, (bs\, \listconcat\ \mkfin(r)) \bigoplus 0,\, c,\, r)\big)^{*}
        & \text{if } m \land \fin(r) \\
    ...
    and $\listconcat$ stands for list concatenation.} 
...
\begin{figure}[ht]
  \begin{center}
    ...
    \begin{tabular}{lcl}
      ...
      $\mkfin(r_1 + r_2)$                    & \dn &
        $\begin{cases}
          \mkfin(r_1) \;\listconcat\; \mkfin(r_2) & \text{if } \fin(r_1) \land \fin(r_2),\\
          \mkfin(r_1)                      & \text{if } \fin(r_1),\\
        ...
        $\mkfin(r_1 \cdot r_2)$ & \dn & \\
          \multicolumn{3}{@{\hspace{15mm}}l}{$
            \begin{cases}
              \mkfin(r_2) \;\listconcat\; \bigl(\mkfin(r_1) \lessdot \mkeps(r_2)\bigr)
