error id: file://<HOME>/Google%20Drive/KCL/Code%20Playground/Marked/marked-SemiringLeftMost.sc:59
      ...
      case BCHAR(_,ch) => 
        ...
        c match {
            case (pos: Int, x: C) => if (x == ch)
                BCHAR(semiring.index(pos) , c)  else BCHAR(semiring.zero , c)
...
def intern[C,S](r: Rexp)(using semiring: SemiringI[S]) : REG[C,S] = r match {
  case ZERO => BZERO()
  case ONE => BONE()
  case CHAR(c) => BCHAR(semiring.zero, c)
...
def intern2[C,S](r: Rexp) : REG[C,S] = BINIT(intern(r))
