error id: file://<HOME>/Google%20Drive/KCL/Code%20Playground/Marked/marked-SemiringLeftMost.sc:24
...
def shift[C,S](mark: S, re: REG[S], c: C)(using semiring: SemiringI[S]): REG[S] = {
    re match {
      case BZERO() => BZERO() //re
      case BONE() => BONE() //re
      case BCHAR(_,ch) => val m=semiring.zero
        //BCHAR(semiring.times(mark , if(ch == c) semiring.one else semiring.zero), c)
        c match {
            case (pos: Int, x: C) => if (x == ch)
                BCHAR(semiring.index(pos) , c)  else semiring.zero
