...
found definition using fallback; symbol length
offset: 2699
...
def matchLength(v: VALUE): Int = v match {
  case EMPTY => 0
  case CHARV(_) => 1
  case SEQV(a, b) => matchLength(a) + matchLength(b)
  case LEFT(v) => matchLength(v)
  case RIGHT(v) => matchLength(v)
  case STARV(vs) => vs.map(matchLength).sum
  case ERRORVALUE(_) => -1
}
