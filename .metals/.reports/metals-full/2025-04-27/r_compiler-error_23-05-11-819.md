...
def shift(m: Boolean, re: MRexp, c: Char, bits:List[Int], pList:List[Color]) : MRexp = {
  ...
  case MCHAR(d,mark) => 
    if (m && d == c)
      MCHAR(d, Mark(marked = true, bs = bits , color = mark.color:::pList))
    else 
  ...
  case MSEQ(r1, r2,p1,p2) if fin(r1) =>
    MSEQ(shift(m, r1, c, bits:+2,p1), shift(true,r2, c,mkfin(r1):+3 ,pList@@),p1,p2) 
  case MSEQ(r1, r2,p1,p2) =>
    MSEQ(shift(m, r1, c, bits:+2,(pList:+Color.RED)), shift(false,r2, c, Nil ,(pList:+Color.GREEN)),p1:::(pList:+Color.RED),p2:::(pList:+Color.GREEN))
...
def test2() = {
  ...
  val finReg=matcher2(rexp,s,List())
  val bits=mkfin(finReg)
  println(s"\n=final list= ${bits}\n")
