...
def fin[C,S](r: REG[C,S])(using semiring: Semiring[S]) : S = r match {
  ...
  case BSTAR(r) => fin(r)
  case BNTIMES(r, n, current) =>  if (current < n) {
    semiring.zero 
}
    else fin(r)
}
def shift[C,S](mark: S, re: REG[C,S], c: C)(using semiring: Semiring[S]): REG[C,S] = {
    re match {
      ...
      case BNTIMES(r, n , current) =>  
        println(s"n is: $n and current is: $current")
        if (current < n) {

                BNTIMES(shift(semiring.plus(mark, fin(r)), r, c), n, current + 1)

            } else {
                BNTIMES(r,n,current)
            }
        
        //if (current > n) BZERO() // Exceeding n repetitions is a failure
            //else if (current == n) BNTIMES(shift(mark, r, c), n, current + 1)
               // else BNTIMES(shift(semiring.plus(mark, fin(r)), r, c), n, current + 1)
            //BSEQ(shift(semiring.plus(mark,fin(r)), r,c) , BNTIMES(r,n-1))   //?????
            //BNTIMES(shift(semiring.plus(mark, fin(r)), r, c), n - 1)
           // BNTIMES(shift(semiring.plus(mark, fin(r)), r, c), n , current + 1)
...
def test01() = {
    ...
    val rexp =NTIMES(SEQ(a,b), 2)
    val regInit=intern2(rexp)

    //val testmyNTIMES= myNtimes(intern(rexp))
    val testmyNTIMES=intern2( SEQ( SEQ( SEQ(a,b) , SEQ(a,b) ) , SEQ(a,b)) )
    println(testmyNTIMES)
    println("\n =========== \n")
    val testntimes2=myNtimes(i@@rexp)
    println(testntimes2)

    val str = "ababab".toList
    println("matcher : reg2 ")
    println(matcher(testmyNTIMES, str))
