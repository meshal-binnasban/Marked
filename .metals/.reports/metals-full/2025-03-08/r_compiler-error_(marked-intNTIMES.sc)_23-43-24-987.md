      ...
      if(n==0) ONE 
        ...
        */
            if (n <= 0) {
                ...
                val shiftedR = shift(mark, r, c)
                @@val markForRest = (mark * nullable(shiftedR)) + fin(shiftedR)

                println(s"shiftedR=$shiftedR + markForRest=$markForRest")
                NTIMES(shiftedR, n, nmark+markForRest)
            }
...
def fin(r: Rexp) : Int = r match {
  ...
  case NTIMES(r, n,nmark) => 
    ...
    // "Tail" means the next (n-1) copies
    val tail = NTIMES(r, n - 1, nmark)
    // Sequence-like formula:
    (fin(r) * nullable(tail)) + fin(tail)
  }
    
   // if(nmark >= n) fin(r) else 0 // ? if (i == 0) false else
}
