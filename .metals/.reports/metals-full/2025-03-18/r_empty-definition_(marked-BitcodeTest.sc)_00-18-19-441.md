error id: file://<HOME>/Google%20Drive/KCL/Code%20Playground/Marked/marked-BitcodeTest.sc:207
...
def decode(r: Rexp, bs: List[Int]): (VALUE, List[Int]) = r match {
  case ONE => (EMPTY, bs) // not sure this should be included
  case CHAR(c, _,) => (CHARV(c), bs) // (2) decode (c) bs = (Char(c), bs)
  case ALTb(r1, r2) => bs match {
    case bs1::Nil => 
        if(bs1 == 0){
