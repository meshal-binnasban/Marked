error id: file://<HOME>/Google%20Drive/KCL/Code%20Playground/Marked/marked-BitcodeTest.sc:173
...
def mkeps_marked2(r: Rexpb): List[Int] = r match {
    ...
    case CHARb(_, marked, bs) => 
        if (marked) {
            println(s"Found marked CHARb, returning: $bs")
            bs // ✅ Stop recursion and return collected bits
        } else {
            println(s"Unmarked CHARb, returning empty")
            List()
        }
    ...
    case ALTb(r1, r2, bs) =>
    if (fin(r1)) {
        val left = mkeps_marked2(r1)
        println(s"taking LEFT path: $left (Prefix: $bs)")
        left  // 🔥 DO NOT append `bs` at this level
    } else if (fin(r2)) {
        val right = mkeps_marked2(r2)
        println(s"taking RIGHT path: $right (Prefix: $bs)")
        bs ++ right // ✅ Only append `bs` here
    } else {
        println(s"ALTb: Neither is final, returning empty")
        bs
    }
