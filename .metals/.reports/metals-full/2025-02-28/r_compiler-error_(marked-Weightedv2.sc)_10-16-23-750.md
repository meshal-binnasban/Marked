...
/*
given semiringIInt: SemiringI[Int] with {
  ...
  case CHAR(c)   => 
    //semiring match {
      //  case si: SemiringI[S] => chari(c)(using si).asInstanceOf[REGW[C, S]]
        //case _: Sem@@iring[S] => charw(x => if (x == c) semiring.one else semiring.zero)
   // }
...
//println(r2)
//println(submatcher(r, "hello athisb isabb test of left most with ab".toList)(using semiringILeftmost))
}
...
/*
def acceptw[C, S](r: REGW[C, S], u: List[C])(using semiring: Semiring[S]): S = {
    r match {
      case ONEw()  => if (u == Nil) semiring.one else semiring.zero
      case CHARw(f) => u match {
        case c :: Nil => f(c)
        case _ => semiring.zero
      }
      case ALTw(r1,r2) => semiring.plus(acceptw(r1, u), acceptw(r2, u))
      case SEQw(r1,r2) =>
        sum(for {
          (u1, u2) <- split(u)
        } yield semiring.times(acceptw(r1,u1), acceptw(r2,u2)))
      case STARw(r) =>
        sum(for {
          ps <- parts(u)
          x = prod(for {
 ...
 def sum[S](l: List[S])(using semiring: Semiring[S]): S = {
    l.foldLeft(semiring.zero)(semiring.plus(_, _))
  }

  def prod[S](l: List[S])(using semiring: Semiring[S]): S = {
    l.foldLeft(semiring.one)(semiring.plus(_, _))
  }
