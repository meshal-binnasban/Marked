...
def fin(r: MRexp) : Boolean = (r: @unchecked) match {
  ...
  case MNTIMES(bs, r, n) => 
    println("here")
    if(n==0) fin(r) else false // new to testX1.
}
...
def shift(m: Boolean, bs: Bits, r: MRexp, c: Char) : MRexp = (r: @unchecked) match {
  ...
  case MNTIMES(cs ,r,n) => MONE(cs)
/*     if(n==0) MONE(cs) 
    else if(fin(r))
    MNTIMES(cs,shift(true, Nil, fuse(cs:::(mkfin(r) :+ Nx),r), c),n-1)
    else
      MNTIMES(cs,shift(m, Nil, fuse(cs,r), c),n-1) */
      
     // MSEQ(Nil,shift(m || fin(r),Nil, fuse(cs,r),c), MNTIMES(Nil,r,n-1)) 
  ...
  //case MNTIMES(cs ,r,n) if m =>  // new to testX1.
  //case MNTIMES(cs ,r,n) =>MNTIMES(Nil,r,n)
}
...
def test1() = {
  ...
  val mrexp=MSEQ(MNTIMES(Nil,"a",2),MNTIMES(Nil,MCHAR(Nil,'a@@),2))
    //intern(rexp)
  val s = "aa".toList
  println(s"Regex:\n${pp(mrexp)}\n")
  println("=string=")
