...
case class MCHAR(c: Char, mark:Boolean) extends MRexp
...
def mkeps(r: Rexp) : Bits = r match {
  ...
  case NTIMES(r, n) => List(En) // new to testX1.
}
...
def fin(r: Rexp) : Boolean = (r: @unchecked) match {
  ...
  case NTIMES(bs, r, n) => fin(r) // new to testX1.
}
...
def mkfin2(r: Rexp) : Set[Bits] = r match {
  ...
  case ALT(r1, r2) if fin(r2) => mkfin2(r2)   
    ...
    mkfin2(r2)
      ...
      if (isStar(r2)){// check if star then no mkeps?
        mkfin2(r1) 
      }else{ 
          println("mkfin2 SEQ") .... }*/ 
  ...
  //case STAR(r) => mkfin2(r)
  case NTIMES(r, n) => mkfin2(r).map(_ ++ List(En)) // new to testX1.
}
