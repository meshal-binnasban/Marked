...
def shift(m: Boolean, bs: Bits, r: MRexp, c: Char) : MRexp = (r: @unchecked) match {
  ...
  case MNTIMES(cs ,r,n) if fin(r) => MSEQ(mkfin(r):+Nx ,shift(true, Nil, fuse(mkfin(r):+Nx,r), c) , MNTIMES(mkfin(r):+Nx,r,n-1)) 
  case MNTIMES(cs ,r,n) if m => MSEQ(cs:+Nx ,shift(true, Nil, fuse(@@r, c) , MNTIMES(cs:+Nx,r,n-1)) 
