...
def shift(m: Boolean, bs: Bits, r: MRexp, c: Char) : MRexp = (r: @unchecked) match {
  ...
  case MSEQ(cs,r1, r2) if fin(r1) => 
    MSEQ(cs,shift(m, Nil, fuse(cs,r1), c), shift(true, Nil, fuse(mkfin(r1),r1), c))
  case MSEQ(cs,r1, r2) => 
    MSEQ(cs,shift(m, Nil, fuse(cs,r1), c), shift(false, Nil, r2, c))

  case MSTAR(cs,r) if m && fin(r)=> MSEQ(cs,shift(true, Nil , fuse(bs:::(mkfin(r) :+ Nx),r), c), MSTAR(cs,r))
  case MSTAR(cs,r) if fin(r)=> MSEQ(cs ,shift(true, Nil , fuse((mkfin(r) :+ Nx),r), c), MSTAR(cs,r))
  case MSTAR(cs,r) if m => MSEQ(cs ,shift(true, Nil, fuse(bs :+ Nx,r), c), MSTAR(cs,r))
  case MSTAR(cs,r)=> MSTAR(Nil,shift(false, Nil, r, c))
  case MNTIMES(cs ,r,n) if n==0 => MONE
  case MNTIMES(cs ,r,n) if m && fin(r) =>SEQ(shift(true, bs:::(mkfin(r) :+ Nx), r, c) , NTIMES(r,n-1)) // new to testX1.
  case MNTIMES(cs ,r,n) if fin(r) => SEQ(shift(true, mkfin(r):+Nx, r, c) , NTIMES(r,n-1)) // new to testX1.
  case MNTIMES(cs ,r,n) if m => SEQ(shift(true, mkfin(r):+Nx, r, c) , NTIMES(r,n-1)) // new to testX1.
  case MNTIME@@S(cs ,r,n) =>NTIMES(shift(false, Nil, r, c),n)
