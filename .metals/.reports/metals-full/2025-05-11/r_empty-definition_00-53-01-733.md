...
def fuse(bs: Bits, r: MRexp) : MRexp = r match {
  ...
  case MPOINT(cs, MCHAR(css,c)) => MPOINT(bs ++ cs, c)
...
def shift(m: Boolean, bs: Bits, r: MRexp, c: Char) : MRexp = (r: @unchecked) match {
  ...
  case MPOINT(_, CHAR(cs,d)) => if (m && d == c) MPOINT(cs, MCHAR(cs,d)) else MCHAR(cs,d)
  ...
  case MNTIMES(cs ,r,n) if m && fin(r) =>MSEQ(cs, shift(true, Nil, fuse(bs:::(mkfin(r) :+ Nx),r), c) , MNTIMES(cs,r,n-1)) // new to testX1.
...
def pp(e: MRexp) : String = (e: @unchecked) match {
  ...
  case MONE => "1\n"
  case MCHAR(cs,c) => s"$c\n"
  case MPOINT(cs, CHAR(css,c)) => s"•$c:${cs.mkString(",")}\n" 
