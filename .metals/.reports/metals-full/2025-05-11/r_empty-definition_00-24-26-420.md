...
case class MCHAR(c: Char) extends MRexp
...
def fuse(bs: Bits, r: MRexp) : MRexp = r match {
  ...
  case MCHAR(c) => MCHAR(c)
  case MPOINT(bs, MCHAR(c)) => MPOINT(bs ++ cs, c)
...
def fin(r: MRexp) : Boolean = (r: @unchecked) match {
  ...
  case NTIMES(bs, r, n) => fin(r) .
}

def mkfin(r: MRexp) : Bits = r match {
  case MPOINT(bs, CHAR(_)) => bs
...
def shift(m: Boolean, bs: Bits, r: MRexp, c: Char) : MRexp = (r: @unchecked) match {
  case MZERO => MZERO
  case MONE => MONE
  case M@@CHAR(d) => if (m && d == c) MPOINT(bs, CHAR(d)) else CHAR(d)
