error id: file://<HOME>/Google%20Drive/KCL/Code%20Playground/Marked/DrChristian/shifts_sets_lexer.sc:`<none>`.
file://<HOME>/Google%20Drive/KCL/Code%20Playground/Marked/DrChristian/shifts_sets_lexer.sc
...
non-local guesses:
	 ...
	 -regenerate/Nx#
	 -regenerate/Nx().
	 -scala/collection/parallel/CollectionConverters.Nx.
	 -scala/collection/parallel/CollectionConverters.Nx#
	 -scala/collection/parallel/CollectionConverters.Nx().
	 -Nx.
	 -Nx#
	 ...
	 -scala/Predef.Nx().
offset: 1879
uri: file://<HOME>/Google%20Drive/KCL/Code%20Playground/Marked/DrChristian/shifts_sets_lexer.sc
text:
```scala
...
def shifts(ms: Marks, r: Rexp): Marks = (r: @unchecked) match {
  ...
  case STAR(r) =>
    ...
    if (ms1.isEmpty) Set() else (ms1 <:+> En) ++ ms1.flatMap(m => shifts(Set(m), STAR(r0)))
...
def matcher(r: Rexp, s: String) : Boolean =
  if s.isEmpty then nullable(r)
  else {
    val im    = Mark(bits = List(), str = s.toList)
    val marks = shifts(Set(im), r)
    println("-------------End of 1 matcher call-----------")
...
def lex(r: Rexp, s: String): Option[Bits] =
  if matcher(r, s) then
    if s.isEmpty then Some(mkeps2(r))
