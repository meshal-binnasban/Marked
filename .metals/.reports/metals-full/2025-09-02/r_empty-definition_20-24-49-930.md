error id: file://<HOME>/Google%20Drive/KCL/Code%20Playground/Marked/DrChristian/play_NMarks_Exp3.sc:collectEmpty.
file://<HOME>/Google%20Drive/KCL/Code%20Playground/Marked/DrChristian/play_NMarks_Exp3.sc
empty definition using pc, found symbol in pc: collectEmpty.
empty definition using semanticdb
empty definition using fallback
non-local guesses:
	 -rexp/ms/collectEmpty.
	 -enumerate/ms/collectEmpty.
	 -regenerate/ms/collectEmpty.
	 -scala/collection/parallel/CollectionConverters.ms.collectEmpty.
	 -ms/collectEmpty.
	 -scala/Predef.ms.collectEmpty.
offset: 2715
...
def shifts(ms: Marks, r: Rexp): Marks = 
  ...
  case ALT(r1, r2) =>  
    ...
    //ms.flatMap { m => (shifts(List(m <:+> Lf), r1) ::: shifts(List(m <:+> Ri), r2))}.prune2         
  case SEQ(r1,r2) => {
    ...
    (nullable(r1), nullable(r2)) match {
      case (true, true) =>
        ...
        val returned= ((r1Consume ::: r1r2Consume).prune2  ::: r2Consume).prune2
        //println(s"ms count=${returned.length} ")
        returned
        //(ms1 <::+>mkeps2(r2)) ::: ms1.flatMap( m => shifts(List(m), r2))  ::: shifts(ms <::+> mkeps2(r1) ,r2) 
      case (true, false) => (ms1.flatMap( m => shifts(List(m), r2))   ::: shifts((ms<::+> mkeps2(r1)),r2)).prune2 
      case (false, true) => ((ms1 <::+>mkeps2(r2)) ::: ms1.flatMap( m => shifts(List(m), r2))).prune2 
      case (false, false) => ms1.flatMap( m => shifts(List(m), r2)).prune2
      } 
  }
  case STAR(r) =>
      val ms1 = shifts(ms<:+>Nx, r).reshuffle.prune2
      if(ms1.isEmpty)
      {
        Nil
      }
      else
        val returned=( (ms1 <:+> En) ::: ms1.flatMap( m=> shifts(List(m), STAR(r)))   )
        //println(s"ms count=${returned.length} ")
        returned.prune2
  ...
  case NTIMES(r,n) =>
    if((ms.colle@@ctEmpty.nonEmpty && n != 0) && !nullable(r)){ 
      Nil
      } else{
