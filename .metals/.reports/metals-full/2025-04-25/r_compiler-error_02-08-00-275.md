...
def mkprio(r: Rexp): Priority = (r: @unchecked) match {
  case POINTS(_, CHAR(_), p) =>
    @@p

  case ALT(r1, r2) =>
    // choose the branch that’s final
    if (fin(r1)) mkprio(r1)
    else           mkprio(r2)

  case SEQ(r1, r2) if fin(r1) && nullable(r2) =>
    // finishing comes from r1
    mkprio(r1)
  ...
  case STAR(inner) =>
    // STAR always finishes via its inner expression
    mkprio(inner)

  case NTIMES(inner, _) =>
    // repetition just defers to the inner
    mkprio(inner)

  case _ =>
    // ZERO, ONE, CHAR(_) have no Priority attached
    // you may want to throw or return a default here
    throw new NoSuchElementException(s"no priority in $r")
}
...
def shift(m: Boolean, bs: Bits, r: Rexp, c: Char, p:Priority) : Rexp = (r: @unchecked) match {
  case ZERO => ZERO
  case ONE => ONE
  case CHAR(d) => if (m && d == c) POINTS(bs:+C,CHAR(d),p) else CHAR(d)
  case POINTS(bits,CHAR(d),p) => if (m && d == c) POINTS(bs:+C,CHAR(d),p.copy(pOrder = p.pOrder + 1)) else CHAR(d)

  case ALT(r1, r2) => ALT(shift(m, bs:+Z , r1, c,p), shift(m, bs:+S, r2, c,p))

  case SEQ(r1, r2) if m && nullable(r1) =>
    SEQ(shift(m, bs:+SE1, r1, c, Priority(A,0)), shift(true,  bs::: (SE1::((mkeps(r1)):+SE2)), r2, c, Priority(B,0))) 
  case SEQ(r1, r2) if fin(r1) =>
     SEQ(shift(m, bs:+SE1, r1, c,p.copy(pOrder = p.pOrder + 1)), shift(true, ((mkfin(r1))):+SE2, r2, c,p.copy(pOrder = p.pOrder + 1)))
  case SEQ(r1, r2) => 
    SEQ(shift(m, bs:+SE1, r1, c,p.copy(pOrder = p.pOrder + 1)), shift(false, Nil, r2, c,p.copy(pOrder = p.pOrder + 1))) //Nil
