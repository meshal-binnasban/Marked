...
def shift(m: Boolean, bs: Bits, r: Rexp, c: Char, p:Priority) : Rexp = (r: @unchecked) match {
  case ZERO => ZERO
  case ONE => ONE
  case CHAR(d) => if (m && d == c) POINTS(bs:+C,CHAR(d),Priority()) else CHAR(d)
  case POINTS(bits,CHAR(d),p) => if (m && d == c) POINTS(bs:+C,CHAR(d),p) else CHAR(d)

  case ALT(r1, r2) => ALT(shift(m, bs:+Z , r1, c,p), shift(m, bs:+S, r2, c,p))

  case SEQ(r1, r2) if m && nullable(r1) =>
    SEQ(shift(m, bs:+SE1, r1, c, Priority(A,1)), shift(true,  bs::: (SE1::((mkeps(r1)):+SE2)), r2, c, Priority(B,1))) 
  case SEQ(r1, r2) if fin(r1) =>
     SEQ(shift(m, bs:+SE1, r1, c,p), shift(true, ((mkfin(r1))):+SE2, r2, c,p))
  case SEQ(r1, r2) => 
    SEQ(shift(m, bs:+SE1, r1, c,p), shift(false, Nil, r2, c,p)) //Nil
...
def mat(r: Rexp, s: List[Char]) : Rexp = s match {
  case Nil => r
  case c::cs => cs.foldLeft(shift(true, Nil, r, c,Priority()))((r, c) => shift(false, Nil, r, c,@@))
}
