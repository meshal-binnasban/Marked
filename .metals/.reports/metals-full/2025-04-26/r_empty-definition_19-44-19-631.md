...
empty definition using pc, found symbol in pc: Rexp#
...
import $file.rexp, rexp._, rexp.Rexp._, rexp.VALUE._, rexp.PriorityType._
...
def mkfin(r: Rexp) : Bits = (r: @unchecked) match {
  case POINTS(bs,CHAR(_),p) => bs
  case ALT(r1, r2) => 
     if (fin(r1) && fin(r2)) {
        val pr1 = mkprio(r1)
        val pr2 = mkprio(r2)
        println(s"pr1=$pr1, pr2=$pr2")
        if (maxPriority(pr1, pr2) == pr1) mkfin(r1)
        else mkfin(r2)
...
def mkprio(r: Rexp): Priority = (r: @unchecked) match {
  case CHAR(_) => Priority()
  case ONE => Priority()
  case POINTS(_, CHAR(_), p) =>p.head
  case ALT(r1, r2) =>if (fin(r1)) mkprio(r1) else mkprio(r2)
  case SEQ(r1, r2) if fin(r1) && nullable(r2) =>mkprio(r1)
  case SEQ(_, r2) =>mkprio(r2)
  case STAR(inner) => mkprio(inner)
  case NTIMES(inner, _) =>mkprio(inner)
}
...
def shift(m: Boolean, bs: Bits, r: Rexp, c: Char, p:Priority) : Rexp = (r: @unchecked) match {
  case ZERO => ZERO
  case ONE => ONE
  case CHAR(d) => if (m && d == c) POINTS(bs:+C,CHAR(d),List(p)) else CHAR(d)
  case POINTS(bits,CHAR(d),ps) => if (m && d == c) POINTS(bs:+C,CHAR(d),p::ps) else CHAR(d) //.copy(pOrder = p.pOrder + 1)

  case ALT(r1, r2) => ALT(shift(m, bs:+Z , r1, c,p), shift(m, bs:+S, r2, c,p))

  case SEQ(r1, r2) if m && nullable(r1) =>
    SEQ(shift(m, bs:+SE1, r1, c, if(p.p == N) Priority(A,1) else p.copy(pOrder = p.pOrder + 1)) , 
    shift(true,  bs::: (SE1::((mkeps(r1)):+SE2)), r2, c, if(p.p == N) Priority(B,1) else p.copy(pOrder = p.pOrder + 1))) 
  case SEQ(r1, r2) if fin(r1) =>
    SEQ(shift(m, bs:+SE1, r1, c, if(p.p == N) Priority(A,1) else p.copy(pOrder = p.pOrder + 1)), shift(true, ((mkfin(r1))):+SE2, r2, c, if(p.p == N | mkprio(r1).p != N) mkprio(r1) else p.copy(pOrder = p.pOrder + 1))) //Priority(B,1)
  case SEQ(r1, r2) => 
    SEQ(shift(m, bs:+SE1, r1, c,if(p.p == N) if(mkprio(r1).p != N) mkprio(r1) else Priority(A,1) else p.copy(pOrder = 7)), shift(false, Nil, r2, c,if(p.p == N) mkprio(r2) else p.copy(pOrder = p.pOrder + 1))) //Nil
  ...
  case STAR(r) if m && fin(r)=>
    STAR(shift(true,(bs):+ST1, r, c,p)) 

  case STAR(r) if fin(r) => 
    STAR(shift(true, mkfinStar(r):+ST1 , r, c,p)) 

  case STAR(r) if m =>
    STAR(shift(m, bs:+ST1, r, c,p))
  case STAR(r) => 
    STAR(shift(false, Nil, r, c,p))
}

def mat(r: Rexp, s: List[Char]) : Rexp = s match {
  case Nil => r
  case c::cs => cs.foldLeft(shift(true, Nil, r, c,Priority()))((r, c) => shift(false, Nil, r, c,Priority()))
}
...
def mDecode(bs: Bits, r: Rexp): (VALUE, Bits) =  (r: @unchecked) match {
  ...
  case @@_ => (ERRORVALUE(s"unmatched bitcode: ${bs}"), bs)
...
def pp(e: Rexp) : String = (e: @unchecked) match {
  ...
  case ONE => s"1\n"
  case CHAR(d) => s"$d \n"
  case POINTS(bs,CHAR(c),p) => s"${"\u001b[32m"} •$c :${bs.mkString(",")} - ${p} ${"\u001b[0m"}\n" 
