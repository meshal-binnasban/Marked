...
def splitPriorityPrefix(bits: Bits): (Bits, Bit, Int) = {
  val (prefix, suffix) = bits.span {
    case A1 | B1 => true
    case _       => false
  }

  if (prefix.nonEmpty) {
    val winner     = prefix.last
    val winnerCount = prefix.count(_ == winner)
...
def mkfin(r: Rexp) : Bits = (r: @unchecked) match {
  ...
  case ALT(r1, r2) => 
     if (fin(r1) && fin(r2)) {
        val (rest1, winner1, count1) = splitPriorityPrefix(mkfin(r1))
...
def shift(m: Boolean, bs: Bits, r: Rexp, c: Char,p:Boolean) : Rexp = (r: @unchecked) match {
  case ZERO => ZERO
  case ONE => ONE
  ...
  case SEQ(r1, r2) if m && nullable(r1) =>
    if(p){
        ...
        val rs2=shift(true,  (A1::bs)::: (SE1::((mkeps(r1)):+SE2)), r2, c,true)
        SEQ( rs1, rs2)
    }else {
  ...
  case SEQ(r1, r2) if fin(r1) =>
    if(p){
    val rs1=shift(m, (A1::bs):+SE1, r1, c ,true)
    val rs2=shift(true, (A1::((mkfin(r1)))):+SE2, r2, c,true)
    SEQ(rs1, rs2) 
    }else {
        val rs1=shift(m, (B1::bs):+SE1, r1, c ,false)
  ...
  case STAR(r) if fin(r) => 
    STAR(shift(true, mkfin(r):+ST1 , r, c,p)) 
...
def mat(r: Rexp, s: List[Char]) : Rexp = s match {
  case Nil => r
  case c::cs => cs.foldLeft(shift(true, Nil, r, c,true))((r, c) => shift(false, Nil, r, c,@@))
}
