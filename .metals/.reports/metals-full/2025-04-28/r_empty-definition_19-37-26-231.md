error id: `<none>`.
file://<WORKSPACE>/play_ComplexMarks.sc
...
non-local guesses:
	 ...
	 -scala/Predef.ONE().
offset: 9530
uri: file://<WORKSPACE>/play_ComplexMarks.sc
text:
```scala
...
@main
def test5() = {
  val rexp=SEQ(ALT(ONE,CHAR('a')),ALT(SEQ(CHAR('a'),CHAR('b')),SEQ(CHAR('b'),ON@@E)))
  println(s"regex= $rexp")
  val s = "ab".toList
  val mrexp=intern2(rexp)
