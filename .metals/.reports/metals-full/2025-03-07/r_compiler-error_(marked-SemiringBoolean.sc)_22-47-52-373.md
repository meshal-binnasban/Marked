      ...
      case BNTIMES(r, n , current) => 
        if (n==0) {
            BONE()
                    ...
                    // if nullable(r) then check if fin(r)
                    val finre=fin(re)
                    println(s"re is $re and fin(re)=$finre")  
                val m=semiring.plus(mark, fin(r)) 
                val shiftedR = shift(m,r,c)
                 //(semiring.times(mark, nullable(shiftedR)), fin(shiftedR))
                val x=fin(shiftedR)
                val y =nullable(shiftedR)
                println(s"m= $m and fin(shiftedR)=$x and nullable(shiftedR) $y")
