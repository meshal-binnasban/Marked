...
non-local guesses:
	 ...
	 -rexp/VALUE.sList#
	 -rexp/VALUE.sList().
	 -play_explicit_bits_2/sList.
	 -play_explicit_bits_2/sList#
	 -play_explicit_bits_2/sList().
	 -derivativesBitcode/sList.
	 -derivativesBitcode/sList#
	 -derivativesBitcode/sList().
	 -sList.
	 -sList#
...
def test2(): Unit = {
  ...
  for (i <- 0L to numRegexes) {
    ...
    for (str <- regenerate.generate_up_to(alphabet)(10)(regex).take(maxStringsPerRegex) if (str!="") ) {
      ...
      val markBitcode: Set[Bits] = lex2(regex, sList).getOrElse(Nil)

      val derivativeR = bders(s@@List, internalize(regex))
      val derivBitcode = bmkeps(derivativeR)
     ...
     // val derivValue=decode(derivBitcode, regex)._1
      ...
      val derivInMarkSet=derivBitcode not in markBitcode
