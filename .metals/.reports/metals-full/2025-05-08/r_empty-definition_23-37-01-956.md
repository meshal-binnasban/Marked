error id: map.
file://<HOME>/Google%20Drive/KCL/Code%20Playground/Marked/DrChristian/play_explicit_bits_N.sc
empty definition using pc, found symbol in pc: map.
empty definition using semanticdb

found definition using fallback; symbol map
offset: 3650
uri: file://<HOME>/Google%20Drive/KCL/Code%20Playground/Marked/DrChristian/play_explicit_bits_N.sc
text:
```scala
//
...
def mkfin2(r: Rexp) : Set[Bits] = r match {
  ...
  case SEQ(r1, r2) if ((fin(r1) && nullable(r2))) => mkfin2(r1).ma@@p(_ ++ mkeps(r2))
...
//SEQ(SEQ(STAR(CHAR(a)),STAR(CHAR(a))),CHAR(a)) 
//( %("a") ~ %("a") ) ~ "a"
@main
def test9() = {
  println("=====Test====")
  ...
  for (i <- (0L to 10_000_000L)) {
    ...
    for (s <- (regenerate.generate_up_to(alphabet)(10)(r).take(9)) if s != "")
      { val v1s = Try(lexer(r, s.toList)).getOrElse(None)
        ...
        if (v1s.isDefined && !v1s.get.contains(v2)) {//add equality for strong test
