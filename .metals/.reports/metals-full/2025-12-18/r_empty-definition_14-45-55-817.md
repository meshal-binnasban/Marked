error id: file://<HOME>/Google%20Drive/KCL/Code%20Playground/Marked/DrChristian/shifts08.sc:filter.
...
non-local guesses:
	 -rexp/initSplits/filter.
	 -rexp/initSplits/filter#
	 -rexp/initSplits/filter().
	 -enumerate/initSplits/filter.
	 -enumerate/initSplits/filter#
	 -enumerate/initSplits/filter().
	 -regenerate/initSplits/filter.
	 -regenerate/initSplits/filter#
	 -regenerate/initSplits/filter().
	 -initSplits/filter.
	 -initSplits/filter#
	 -initSplits/filter().
	 -scala/Predef.initSplits.filter.
	 -scala/Predef.initSplits.filter#
	 -scala/Predef.initSplits.filter().
offset: 6566
...
def filterSplits(
  ...
  val Mark(n, m) = p
  val inRange = initSplits.f@@ilter(k => k >= n && k <= m)
