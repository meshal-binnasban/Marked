...
enum Rexp {
  ...
  case STAR(r: Rexp)
  case NTIMES(r: Rexp, n: Int,nmark:Int =0 , innerR :Rexp ) 
  case INIT(r: Rexp)
  ...
  override def toString: String = this match {
    ...
    case NTIMES(r, n, nmark,_) => s"NTIMES($r, n=$n, nmark=$nmark)"
    ...
    case NTIMES(r, n,nmark , innerR) =>
      ...
      if(n==0) ONE 
        ...
        if (n==0) ONE  // already matched n times
                ...
                val shiftedR = shift(mark+fin(r),r,c)
                    if (fin(shiftedR) == 1) NTIMES(innerR, n, nmark + 1,innerR)
                        else NTIMES(shiftedR, n, nmark,innerR)
      }
      */
...
def nullable(r: Rexp) : Int = r match {
  ...
  case STAR(_) => 1
  case NTIMES(r, n ,nmark , _) => if (n == 0) 1 else nullable(r) //?
...
def fin(r: Rexp) : Int = r match {
  ...
  case STAR(r) => fin(r)
  case NTIMES(r,n,nmark,innerR) => if(n == nmark) 1 else 0
...
def size(r: Rexp) : Int = r match {
  ...
  case NTIMES(r,n, nmark,innerR) => 1 + size(r) 
...
def SeqNTIMES(r: Rexp): Rexp = r match {
  ...
  case NTIMES(r, n, _ , _) =>
    ...
    if (n <= 0) ONE else
        if (n == 1) expandR else 
            { SeqNTIMES( SEQ( expandR, NTIMES(expandR,n-1,0,ONE) ) ) }
}


def EVIL1(n: Int) = SEQ(NTIMES(OPT(CHAR('a')), n,0,OPT(CHAR('a'))), NTIMES(CHAR('a'), n,0,CHAR('a')))
...
def pp(e: Rexp) : String = e match {
  ...
  case SEQ(r1, r2) => "SEQ\n" ++ pps(r1, r2)
  case STAR(r) => "STAR\n" ++ pps(r)
  case NTIMES(r,n,nmark,_) => if (nmark == n) s"• NTIMES(n=$n) $nmark\n" ++ pps(r) 
