error id: file://<HOME>/Google%20Drive/KCL/Code%20Playground/Marked/marked-intNTIMES.sc:112
...
def mkeps_marked(r: Rexp): VALUE = r match {
    case ONE => EMPTY
    case CHAR(c,marked) => if(marked >=1) CHARV(c)
      else None
    case ALT(r1, r2) =>
      if (fin(r1) == 1) Some(Left(mkeps_marked(r1)))
      else if (fin(r2)) Some(Right(mkeps_marked(r2)))
      else sys.error("ALT does not match empty")

    case SEQ(r1, r2) => SeqVal(mkeps_marked(r1), mkeps_marked(r2))

    case STAR(r1) =>
      if (fin(r1)) Stars(List(mkeps_marked(r1)))
      else Stars(Nil)

    case NTIMES(_, n, count) =>
      if (count == n) Empty else sys.error("NTIMES does not match empty")

    case ONE => Empty
    case ZERO => sys.error("ZERO does not match empty")
    case _ => sys.error("No match for mkeps_marked")
  }
