...
def fin[C,S](r: REG[C,S])(using semiring: Semiring[S]) : S = r match {
  ...
  case BSEQ(r1, r2) =>{
    val finr1=fin(r1)
    val nullr2=nullable(r2)
    val finr2=fin(r2)
//println(s"in fin SEQ,semiring.plus(semiring.times(fin(r1 $finr1),nullable(r2 $nullr2)),fin(r2 $finr2))")
    semiring.plus(semiring.times(fin(r1),nullable(r2)),fin(r2))
  }
  case BSTAR(r) => fin(r)
  case BNTIMES(r, n, current) => 
   // println(s"current at fin is $current \n")
    if(current == n)
    {
        val x= fin(r)
     //   println(s"fin r at fin btimes = $x")
        x
    }else
        {val y=semiring.times(fin(r), nullable(r))
        println(s"semiring.times(fin(r), nullable(r)) = $y")
        y
    }
    
    
  /*
    if ((nullable(r) == semiring.one && current !=n) ){
    semiring.one
    }  else if (current == n) {
        fin(r)
    }else {
    ...
    re match {
      ...
      case BNTIMES(r, n , current) => 
        if(n==0) BONE()
            ...
            shift(semiring.plus(mark, fin(r)),BNTIMES(r,n,current+1),c)
             }else
                {
                BNTIMES(r, n , current)
                }@@
