...
def fin[C,S](r: REG[C,S])(using semiring: Semiring[S]) : S = r match {
  ...
  case BNTIMES(r, n, current) =>  
    if (current == n && fin(r) == semiring.one) semiring.one
    else semiring.zero
}
def shift[C,S](mark: S, re: REG[C,S], c: C)(using semiring: Semiring[S]): REG[C,S] = {
    re match {
      ...
      case BNTIMES(r, n , current) => 
        if (current < n) {
          // Shift the mark into the inner regex
          val shiftedR = shift(semiring.plus(mark, fin(r)), r, c)
          // Check if the inner regex has matched successfully
          if (fin(shiftedR) == semiring.one) {
            // Increment current only if the inner regex matches
            BNTIMES(shiftedR, n, current + 1)
          } else {
            // Otherwise, keep current unchanged
            BNTIMES(shiftedR, n, current)
          }
        } else if (current == n) {
          // When current == n, we stop shifting and return the final state
          BNTIMES(r, n, current)
        } else {
          // If current > n, return the original regex (should not happen)
          re@@
        }
