error id: file://<HOME>/Google%20Drive/KCL/Code%20Playground/Marked/DrChristian/play_NMarks_Exp3.sc:reshuffle.
...
found definition using fallback; symbol reshuffle
offset: 1362
...
def shifts(ms: Marks, r: Rexp): Marks = 
    ...
    if (ms == Nil) Nil
        ...
        case SEQ(r1,r2) => {
            val ms1 = shifts(ms, r1).re@@shuffle
            (nullable(r1), nullable(r2)) match {
                case (true, true) =>  (shifts(ms1 ::: (ms <::+> mkeps2(r1)), r2) ::: (ms1 <::+> mkeps2(r2)))
                case (true, false) => shifts(ms1 ::: (ms<::+> mkeps2(r1)), r2)
                case (false, true) => shifts(ms1, r2) ::: (ms1 <::+>mkeps2(r2))
                case (false, false) => shifts(ms1, r2)
        }
      }
        case STAR(r) =>
            val ms1 = shifts(ms<:+>Nx, r)
            ( shifts(ms1, STAR(r)) ::: (ms1 <:+> En)  )
...
def lex(r: Rexp, s: String): Option[Bits] =
  if matcher(r, s) then
    ...
    else Some(shifts(List(Mark(bits = List(), str = s.toList)), r).head.bits)
  else None
