error id: file://<HOME>/Google%20Drive/KCL/Code%20Playground/Marked/DrChristian/play_NMarks_Exp3.sc:flatMap.
...
non-local guesses:
	 ...
	 -rexp/ms1/flatMap#
	 -rexp/ms1/flatMap().
	 -enumerate/ms1/flatMap.
	 -enumerate/ms1/flatMap#
	 -enumerate/ms1/flatMap().
	 -regenerate/ms1/flatMap.
	 -regenerate/ms1/flatMap#
	 -regenerate/ms1/flatMap().
	 -scala/collection/parallel/CollectionConverters.ms1.flatMap.
	 -scala/collection/parallel/CollectionConverters.ms1.flatMap#
	 -scala/collection/parallel/CollectionConverters.ms1.flatMap().
	 -ms1/flatMap.
	 -ms1/flatMap#
	 -ms1/flatMap().
	 -scala/Predef.ms1.flatMap.
	 -scala/Predef.ms1.flatMap#
	 -scala/Predef.ms1.flatMap().
offset: 1988
...
def shifts(ms: Marks, r: Rexp): Marks = 
    ...
    if (ms == Nil) Nil
        ...
        case ALT(r1, r2) =>
            ( (ms<:+> Lf).flatMap( m => shifts(List(m) , r1)) ::: (ms<:+> Ri).flatMap( m => shifts(List(m) , r2)) )
        case SEQ(r1,r2) => {
            val ms1 = shifts(ms, r1)
            (nullable(r1), nullable(r2)) match {
                case (true, true) =>  shifts(ms1, r2) ::: (ms1 <::+> mkeps2(r2)) ::: shifts( (ms <::+> mkeps2(r1)),r2) 
                case (true, false) => shifts(ms1 , r2) ::: shifts( (ms<::+> mkeps2(r1)), r2)
