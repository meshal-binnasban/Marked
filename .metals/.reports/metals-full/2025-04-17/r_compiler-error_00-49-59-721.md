...
def totalBitsWeight(bs: Bits): Double = {
  ...
  (weightedSum / cCount) * 100.0 // average reward per C, scaled to percentage
}

def weightedCDistance(bs: Bits): Double = {
  val cIndices = bs.zipWithIndex.collect { case (C, idx) => idx }

  // If there's only one or zero Cs, no distance to compute
  if (cIndices.length <= 1) return 0.0

  // Compute weighted distances between Cs
  cIndices.sliding(2).map {
    case List(a, b) =>
      val distance = b - a
      val weight = 1.0 / (a + 1)  // earlier 'a' gets more weight
      distance * weight@@
  }.sum
}
