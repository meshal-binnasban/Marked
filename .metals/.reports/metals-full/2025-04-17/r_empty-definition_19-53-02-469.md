...
non-local guesses:
	 ...
	 -rexp/VALUE.loop#
	 -rexp/VALUE.loop().
	 -derivativesBitcode/loop.
	 -derivativesBitcode/loop#
	 -derivativesBitcode/loop().
	 -scala/math/Ordering.Implicits.loop.
	 -scala/math/Ordering.Implicits.loop#
	 -scala/math/Ordering.Implicits.loop().
	 -loop.
	 -loop#
...
def mkeps(r: Rexp) : Bits = (r: @unchecked) match {
  ...
  case STAR(r) => (ST1::mkeps(r)):+ST2 // if nullable then st1 List(E) st2?
...
def mkfin(r: Rexp) : Bits = (r: @unchecked) match {
  ...
  case ALT(r1, r2) => 
    if (fin(r1) && fin(r2)) {
    println(s"mkfin1=${mkfin(r1)}, mkfin2=${mkfin(r2)}\ntotalBitsWeight r1 ${progressiveBitScore(mkfin(r1))} and r2 ${progressiveBitScore(mkfin(r2))}")
     if (progressiveBitScore(mkfin(r1))>=progressiveBitScore(mkfin(r2)))
...
def shift(m: Boolean, bs: Bits, r: Rexp, c: Char) : Rexp = (r: @unchecked) match {
  ...
  //case STAR(r) if m && fin(r) =>
    // {println(s"r=$r , bs=${ bs ::: ((mkfin(r) :+ST1))} ,mkfin r=${mkfin(r)}")
...
def bdecode(bs: Bits): (VALUE, Bits) = bs match {
  ...
  case ST1 :: rest =>
    println(s"ST1 $ST1 :: rest $rest")
    def loop(bits: Bits, vs: List[VALUE]): (VALUE, Bits) = bits match {
      case ST2 :: rest => (STARV(vs.reverse), rest)
      case ST1 :: bsv =>
        val (before, after) = bsv.span(_ != ST1)
        println(s"before=$before, after=$after")
        if(!after.isEmpty) {
          val (v, bsvp) = bdecode(before)
          println(s"v=$v, bsvp=$bsvp")
          loop(after, v :: vs)
        }            if(after.isEmpty) {
            @@loop(bsvp, v :: vs)
          } else
      case _ =>
        (ERRORVALUE(s"STAR ERROR, bs=$bits"), bits)
    }
...
def test7() = {
  ...
  println(s"===============\nFinal Reg:\n ${pp(finReg)} \nnullable=${nullable(finReg)}")
  println(s"===============\nMarked bitcode: ${bits} \nMarked bitcode converted: ${convertMtoDBit(bits)}")
 // println(s"Decoded value for Marked=${markedValue}")
  ...
  //println(s"===============\ncompareResults = ${compareResults(markedValue, derivValue)}")
  println(s"===============\nmarked bitcode == derivatives bitcode :  ${(bitsToInts(convertMtoDBit(bits)) == derivBitcode)}")
