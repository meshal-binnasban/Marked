...
### dotty.tools.dotc.core.TypeError$$anon$1: Toplevel definition test4 is defined in
...
def shift(m: Boolean, r: Rexp, c: Char) : Rexp = {
  ...
  case POINT(NTIMES(r, n,counter)) => r
  case NTIMES(r, n,counter) => 
    if (counter == n) NTIMES(r, n) else 
        if (m || fin(r)) {
...
def test1() = {
  ...
  println(s"Result= ${matcher(reg, ss)}")
  val finReg2=matcher2(reg, ss)
