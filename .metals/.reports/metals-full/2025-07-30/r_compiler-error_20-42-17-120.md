...
/*             if(nullable(r1)){
        ...
        case STAR(r, m) =>
          // add mkeps other wise stop? and also check m
          if(nullable(r)) {
            if(m){
            val (rms, cr) = shift((ms <:+> Nx).reshuffle, r, consumed)
            
            if(cr){

            }e@@
           // val (sms,cs) = shift(rms,STAR(r,false),cr)
            //println(s"rms=$rms, sms=$sms")
            ( (((ms<:+>En) ::: (onlyMarks <:+> En)  ).reshuffle ) ,cr) 

            }else{
            val (rms, cr) = shift(ms <:+> Nx, r, consumed)
            val (sms,cs) = shift(rms.reshuffle,STAR(r,false),cr)
            //println(s"rms=$rms, sms=$sms")
            ( (((ms<:+> En) ::: (sms ) )).reshuffle ,cs) 
            }
           
          }else {
            val (rms, cr) = shift(ms <:+> Nx, r, consumed)
            val (sms,cs) = shift(rms.reshuffle,STAR(r),cr)
            //println(s"rms=$rms, sms=$sms")
            ((((ms<:+>En) ::: sms  ).reshuffle ),cs)     
          } 
