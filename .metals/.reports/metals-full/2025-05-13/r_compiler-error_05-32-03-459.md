...
def shift(m: Boolean, bs: Bits, r: MRexp, c: Char) : MRexp = (r: @unchecked) match {
 ...
 // case MSTAR(cs,r) if fin(r)=> MSEQ(cs ,shift(true, Nil , fuse((mkfin(r) :+ Nx),r), c), MSTAR(cs,r))
  ...
  case MNTIMES(cs ,r,n) if m => 
    if(n-1 ==0)
    MSEQ(Nil ,shift(true, Nil, fuse(cs:+Nx,r), c) , MONE(@@))
    else
      MSEQ(Nil ,shift(true, Nil, fuse(cs:+Nx,r), c) , MNTIMES(Nil,r,n-1))
...
def basic1() = {
  println("=====Test====")
  //val rexp = "a" ~ "b" | "c"
  val rexp=OPT(NTIMES("a",2)) ~ ( OPT(NTIMES("ab",2)) ~ OPT(NTIMES("b",2)) )
