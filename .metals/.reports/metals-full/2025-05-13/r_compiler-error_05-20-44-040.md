...
def shift(m: Boolean, bs: Bits, r: MRexp, c: Char) : MRexp = (r: @unchecked) match {
  ...
  case MSEQ(cs,r1, r2) if m && mnullable(r1) => 
    ...
    MALT( MSEQ(shift(true, Nil, fuse(cs,r1), c), shift(fin(r), Nil , fuse(cs:::mkeps(r1),r2), c)) 
    , shift(true,bs:::mkeps(r1), fuse(,@@r2), c) )
 ...
 // case MNTIMES(cs ,r,n) if fin(r) =>MSEQ(cs ,shift(true, Nil, fuse(mkfin(r),r), c) , MNTIMES(mkfin(r),r,n-1)) 

  case MNTIMES(cs ,r,n) if m => MSEQ(cs ,shift(true, Nil, fuse(cs:+Nx,r), c) , MNTIMES(cs,r,n-1)) 
