error id: file://<HOME>/Google%20Drive/KCL/Code%20Playground/Marked/marked-Weightedv1.sc:95
...
def matcher[C, S](r: REGW[C, S], s: List[C])(using semiring: Semiring[S]): S = {
  ...
  def shift[C, S](mark: S, re: RE[C, S], c: C)(using semiring: Semiring[S]): REGW[C, S] = {
    re match {
      case ONE() => one
      case CHAR(f) => char(f).copy(finalw = semiring.times(mark, f(c))) // creates a copy of the same REGW but with new finalw value
...
val a = char(_ == 'a')(using booleanSemiring)
val b= char(_ == 'b')(using booleanSemiring)
val reg=seq(a,b)(using booleanSemiring)

println("testing new implementation")
matcher(reg, "ab".toList)
