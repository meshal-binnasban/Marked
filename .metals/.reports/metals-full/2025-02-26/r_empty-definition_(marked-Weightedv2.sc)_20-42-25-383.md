...
empty definition using pc, found symbol in pc: index.
...
def chari[C, S](c: Char)(using semiring: Semiring[S]): REGW[(Int, C), S] = {
    val semiI = summon[SemiringI[S]]
    val semiring = summon[Semiring[S]]
    def weight(t: (Int, Char)): S = {
      val (pos, x) = t
      if (x == c) semiI.index(pos)
      else semiring.zero
    }
    charw(weight)
}
...
val a = chari('a')
val ab = starw(altw(a, chari('b')))
val aaba = seqw(a, seqw (ab, a))

    println(submatcher(a, "bbbbaa".toList))
}
