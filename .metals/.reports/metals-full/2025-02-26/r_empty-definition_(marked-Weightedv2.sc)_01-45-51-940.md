error id: file://<HOME>/Google%20Drive/KCL/Code%20Playground/Marked/marked-Weightedv2.sc:32
...
def shift[C,S](mark: S, r: Rexpw[C,S], c: C)(using semiring: Semiring[S]): Rexpw[C,S] = r match {
    case ZEROw => ZEROw
    case ONEw=> ONEw
    case CHARw(f,fin) =>  CHARw(f,semiring.times(mark, f(c))) // must add final variable for it to change when called in final function or?
