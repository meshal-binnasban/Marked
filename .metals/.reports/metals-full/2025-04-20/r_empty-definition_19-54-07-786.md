error id: _empty_/VALUE.ERRORVALUE.
...
non-local guesses:
	 ...
	 -VALUE.ERRORVALUE#
	 -VALUE.ERRORVALUE().
	 -scala/math/Ordering.Implicits.ERRORVALUE.
	 -scala/math/Ordering.Implicits.ERRORVALUE#
	 -scala/math/Ordering.Implicits.ERRORVALUE().
	 -Rexp.ERRORVALUE.
	 -Rexp.ERRORVALUE#
...
implicit val listIntOrdering: Ordering[List[Int]] = Ordering.Seq[Int]

def flattenMatchConsumption(v: VALUE, depth: Int = 0): List[Int] = v match {
  case EMPTY => Nil
  case CHARV(_) => List(depth)
  case LEFT(v1) => flattenMatchConsumption(v1, depth + 1)
  case RIGHT(v2) => flattenMatchConsumption(v2, depth + 1)
  case SEQV(a, b) => flattenMatchConsumption(a, depth) ++ flattenMatchConsumption(b, depth)
  case STARV(vs) => vs.flatMap(v => flattenMatchConsumption(v, depth + 1))
  case ERR@@ORVALUE(_) => List(Int.MaxValue)
}

def posixCompare(v1: VALUE, v2: VALUE): Boolean = {
  val c1 = flattenMatchConsumption(v1)
  val c2 = flattenMatchConsumption(v2)
