...
def unshift(m: Int, r: Rexp, c: Char): Rexp = r match {
  case ZERO => ZERO
  case ONE => ONE
  case CHAR(d,makred) => 
    // Undo m && (d == c) from shift
    CHAR(marked.head >= 1 || d != c, c)
  
  case BALT(r1, r2) => 
    // Since shift propagates m to both branches, reverse it by merging marks
    BALT(unshift(m, r1, c), unshift(m, r2, c))
  
  case BSEQ(r1, r2) =>
    if (nullable(r1) && r2 == shift(true, r2, c)) {
      // If shift introduced an ALT, restore the original SEQ
      BSEQ(unshift(m, r1, c), unshift(fin(r1), r2, c))
    } else {
      // Otherwise, reverse the standard propagation
      BSEQ(unshift(m, r1, c), unshift(fin(r1), r2, c))
    }
  
  case BSTAR(r) =>
    // If shift applied m || fin(r), we must restore original m
    BSTAR(unshift(m && !fin(r), r, c))
  
  case BNT(r, n) =>
    if (n == 0) BNT(r, n) 
    else {
      val prev_m = (n + 1 > 0) && (m || !fin(r)) // Reverse m || fin(r)
      BNT(unshift(prev_m, r, c), n + 1) // Increment n back
    }
}
