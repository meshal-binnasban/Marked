error id: file://<HOME>/Google%20Drive/KCL/Code%20Playground/Marked/play_point3_BIT.sc:60
file://<HOME>/Google%20Drive/KCL/Code%20Playground/Marked/play_point3_BIT.sc
...
def shift(m: Boolean, re: Rexp, c: Char): Rexp = re match {
  case BIT(inner, bs) => inner match {
    case ZERO => BIT(ZERO, bs)
    case ONE  => BIT(ONE, bs)
    case CHAR(d) =>
    ...
    case ALT(r1, r2) =>
      val leftShifted = shift(m, BIT(r1, List(0) ++ bs), c)
      val rightShifted = shift(m, BIT(r2, List(1) ++ bs), c)
      ALT(leftShifted,rightShifted)

    case SEQ(r1, r2) =>
      val shiftedR1 = shift(m, BIT(r1, bs), c)
      val shiftedR2 = shift((m && nullable(r1)) || fin(r1), BIT(r2, bs), c)
      BIT(SEQ(extractR(shiftedR1), extractR(shiftedR2)), extractBits(shiftedR2))

    case STAR(r1) =>
      val shiftedInner = shift(m || fin(r1), BIT(r1, bs), c)
      // If an iteration occurs, prepend 0
      if (fin(shiftedInner)) fuse(List(0), BIT(STAR(extractR(shiftedInner)), extractBits(shiftedInner)))
      else BIT(STAR(extractR(shiftedInner)), extractBits(shiftedInner))

    case NTIMES(r1, n, counter) =>
      if (counter == n) BIT(NTIMES(r1, n, counter), bs)
      else {
        val nextShifted = if (m || fin(r1))
          shift(true, BIT(r1, bs), c)
        else
          shift(false, BIT(r1, bs), c)

        BIT(NTIMES(extractR(nextShifted), n, if (m || fin(r1)) counter + 1 else counter), extractBits(nextShifted))
      }

    case BIT(_, _) => throw new RuntimeException("Nested BIT nodes are not allowed")
  }

  case _ => throw new RuntimeException("Top-level shift must be BIT-wrapped")
}
