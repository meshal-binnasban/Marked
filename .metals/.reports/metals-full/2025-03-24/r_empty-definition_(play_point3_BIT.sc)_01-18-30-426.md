error id: file://<HOME>/Google%20Drive/KCL/Code%20Playground/Marked/play_point3_BIT.sc:55
...
def shift(m: Boolean, re: Rexp, c: Char,bs: List[Int]): Rexp = re match {
    ...
    case ALT(r1, r2) =>
      val leftShifted = shift(m,r1,c,  List(0) ++ bs)
      val rightShifted = shift(m,r2,c, List(1) ++ bs)
    ...
    case NTIMES(r, n, counter) =>
      if (counter == n) BIT(NTIMES(r, n, counter), bs)
      else {
        if (m || fin(r)){
          BIT( NTIMES(shift(m || fin(r), r, c), n,counter+1), bs)
        }
        else
          BIT( NTIMES(shift(false, r, c), n,counter+1), bs)
      }
}
