...
def mkfin(r: MRexp) : Bits = r match {
  ...
  case MSEQ(bs ,r1, r2) if fin(r1) && fin(r2) => mkfin(r1) ++ mkfin(r2)
...
def shift(m: Boolean, bs: Bits, r: MRexp, c: Char) : MRexp = (r: @unchecked) match {
 ...
 // case MNTIMES(cs ,r,n) if fin(r) => MSEQ(Nil ,shift(true, Nil, fuse(mkfin(r),r), c) , MNTIMES(Nil,r,n-1)) 
  case MNTIMES(cs ,r,n) if m => MNTIMES(MSEQ(Nil ,shift(true, Nil, fuse(cs:+Nx,r), c) , MNTIMES(Nil,r,n-1)) ,@@)
