error id: ammonite/$file/`marked-SemiringLeftMost`.`~`().(s)
...
def test2() = {
    ...
    val a = CHAR(helperft('a')(using semiringILeftmost))
    val b = CHAR(helperft('b')(using semiringILeftmost))
    val reg =STAR(SEQ(a, b))

    val reg2=intern(reg)(using semiringILeftmost)

    val str = "aba".toList.zipWithIndex
    println("match reg2 : using intern")
    println(matcher(reg2, str)(using semiringILeftmost))
