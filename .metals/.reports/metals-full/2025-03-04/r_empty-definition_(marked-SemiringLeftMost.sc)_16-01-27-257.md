...
non-local guesses:
	 -Rexp.semiringILeftmost.
	 -Rexp.semiringILeftmost#
	 -Rexp.semiringILeftmost().
	 -REG.semiringILeftmost.
	 -REG.semiringILeftmost#
	 -REG.semiringILeftmost().
	 -semiringILeftmost.
	 -semiringILeftmost#
	 -semiringILeftmost().
	 -scala/Predef.semiringILeftmost.
	 -scala/Predef.semiringILeftmost#
	 -scala/Predef.semiringILeftmost().
...
def test1() = {
    ...
    val func= (x:Char,y:Char) => if (x == y) semiringILeftmost.one else semiringILeftmost.zero

    val br1 = SEQ(CHAR(func(_,'a')), SEQ(CHAR(func(_,'b')), CHAR(func(_,'c'))))
    val br2 = intern2(br1)
...
def test2() = {
  ...
  val func= (x:Char, pos:Int, c:Char) => if (x == c) semiringILeftmost.index(pos) else semiringILeftmost.zero
  val a = CHAR[(Char, Int), semiringILeftmost.type](using semiringILeftmost)(func(_,_,'a'))
  val b = CHAR[(Char, Int), semiringILeftmost.type](using semiringILeftmost)(func(_,_,'b'))
  val reg = SEQ(a, b)
  val str = "ab".toList.zipWithIndex

  println("testing new implementation")
  println(matcher(intern(reg)(using semiringILeftmost), str))
