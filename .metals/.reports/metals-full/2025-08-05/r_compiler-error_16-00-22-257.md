...
### scala.MatchError: TypeDef(Marks,AppliedTypeTree(Ident(List),List(Ident(Mark)))) (of class dotty.tools.dotc.ast.Trees$TypeDef)
...
def test2() = {
  ...
  println(s"derivBits: ${derivBits}\n------------------------")
  println(s"derivVal: ${derivVal}")
  lexerMarks(br2,s).getOrElse(Nil).foreach(v => println(s"Marked Values:$v") )
  println("------------------------")
  if(markedBits.getOrElse(Nil) == derivBits) 
  println("Matched Derivative")
  else
