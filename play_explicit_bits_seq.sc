...
def mkeps(r: MRexp) : Bits = r match {
  ...
  case MSTAR(bs ,r) => bs++List(En)
  case MNTIMES(bs ,r, n) => List(En)
    //if(n==0) List(En) else Nx :: (mkeps(r) ++ mkeps(MNTIMES(r,n-1)) )  // new to testX1.
...
def shift(m: Boolean, bs: Bits, r: MRexp, c: Char) : MRexp = (r: @unchecked) match {
  ...
  case MPOINT(cs, MCHAR(css,d)) => if (m && d == c) MPOINT(cs, MCHAR(Nil,d)) else MCHAR(Nil,d)
  
  //case MPOINT(bss, MONE(cs)) =>  MPOINT(cs,MONE(cs))
  
  case MALT(cs ,r1, r2) => MALT(Nil,shift(m, Nil, fuse(cs:+Lf, r1), c), shift(m, Nil, fuse(cs:+Ri, r2), c))

  case MSEQ(cs,r1, r2) if m && mnullable(r1) => 
    MALT(Nil, MSEQ(Nil,shift(true, Nil, fuse(cs,r1), c), shift(false, Nil, fuse(cs++mkeps(r1),r2), c)) //
  ...
  case MSTAR(cs,rs) if m => MSEQ(Nil ,shift(true, Nil, fuse(cs ++ List(Nx),rs), c), MSTAR(Nil,rs))
  case MSTAR(cs,r)=> MSTAR(Nil,shift(false, Nil, r, c))

  case MNTIMES(cs ,r,n) if n <=0 =>MONE(List(En)) //MPOINT(List(En), MONE(List(En)))
  //case MNTIMES(cs, r, n) if fin(r) => MSEQ(Nil ,shift(true, Nil, fuse(cs:+Nx,r), c) , MNTIMES(List(En) ,r,n-1))
  case MNTIMES(cs ,r,n) if m => MSEQ(Nil ,shift(true, Nil, fuse(cs:+Nx,r), c) , MNTIMES(Nil ,r,n-1))
